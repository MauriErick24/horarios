const schedule = {
  horario: [
    {
      carrera: "LICENCIATURA EN INGENIERIA INFORMATICA",
      semestre: [
        {
          nivel: "1",
          materia: [
            {
              nombre: "INGLES I",
              docentes: [
                {
                  nombre: "CESPEDES GUIZADA MARIA BENITA",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "693B",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "691D",
                    },
                  ],
                },
                {
                  nombre: "CESPEDES GUIZADA MARIA BENITA",
                  grupo: "2",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "690D",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "690D",
                    },
                  ],
                },
                {
                  nombre: "PEETERS ILONAA MAGDA LENA",
                  grupo: "3",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "691B",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "692H",
                    },
                  ],
                },
                {
                  nombre: "GRILO SALVATIERRA MARIA ESTELA",
                  grupo: "4",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "15:45",
                      horaFin: "18:45",
                      aula: "692G",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "692E",
                    },
                  ],
                },
                {
                  nombre: "CESPEDES GUIZADA MARIA BENITA",
                  grupo: "5",
                  dia: [
                    {
                      nombre: "Jueves",
                      horaInicio: "09:00",
                      horaFin: "10:30",
                      aula: "692F",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "691B",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "FISICA GENERAL",
              docentes: [
                {
                  nombre: "VALENZUELA MIRANDA ROBERTO",
                  grupo: "B",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "612",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "692C",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "ALGEBRA I",
              docentes: [
                {
                  nombre: "RODRIGUEZ SEJAS JUAN ANTONIO",
                  grupo: "10",
                  dia: [
                    {
                      nombre: "Miércoles",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "692E",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "692F",
                    },
                  ],
                },
                {
                  nombre: "CARRASCO CALVO ALVARO HERNANDO",
                  grupo: "15",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "607",
                    },
                    {
                      nombre: "Lunes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "642",
                    },
                  ],
                },
                {
                  nombre: "LEON ROMERO GUALBERTO",
                  grupo: "8",
                  dia: [
                    {
                      nombre: "Jueves",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "625D",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "607",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "CALCULO I",
              docentes: [
                {
                  nombre: "ROJAS ZURITA RAMIRO",
                  grupo: "11",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "622",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "690B",
                    },

                    {
                      nombre: "Jueves",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "692E",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "INTRODUCCION A LA PROGRAMACION",
              docentes: [
                {
                  nombre: "SALAZAR SERRUDO CARLA",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Jueves",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "691A",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "691E",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "623",
                    },
                  ],
                },
                {
                  nombre: "COSTAS JAUREGUI VLADIMIR ABEL",
                  grupo: "10",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "693D",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691A",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "691B",
                    },
                  ],
                },
                {
                  nombre: "BLANCO COCA LETICIA",
                  grupo: "2",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "617",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "691B",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "624",
                    },
                  ],
                },
                {
                  nombre: "USTARIZ VARGAS HERNAN",
                  grupo: "3",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "607",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "612",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "624",
                    },
                  ],
                },
                {
                  nombre: "VILLARROEL TAPIA HENRY FRANK",
                  grupo: "4",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "623",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "693C",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "17:15",
                      horaFin: "17:15",
                      aula: "691E",
                    },
                  ],
                },
                {
                  nombre: "MONTANO QUIROGA VICTOR HUGO",
                  grupo: "5",
                  dia: [
                    {
                      nombre: "Miércoles",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "690B",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "623",
                    },
                    {
                      nombre: "Sábado",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691C",
                    },
                  ],
                },
                {
                  nombre: "SALAZAR SERRUDO CARLA",
                  grupo: "6",
                  dia: [
                    {
                      nombre: "Miércoles",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "692D",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "691D",
                    },
                    {
                      nombre: "Sábado",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "661",
                    },
                  ],
                },
                {
                  nombre: "ANTEZANA SERRANO DILAN ALEJANDRO",
                  grupo: "7",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "690C",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "692E",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "693B",
                    },
                  ],
                },
              ],
            },
          ],
        },
        {
          nivel: "2",
          materia: [
            {
              nombre: "INGLES II",
              docentes: [
                {
                  nombre: "PEETERS ILONAA MAGDA LENA",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691B",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "661",
                    },
                  ],
                },
                {
                  nombre: "PEETERS ILONAA MAGDA LENA",
                  grupo: "2",
                  dia: [
                    {
                      nombre: "Jueves",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691B",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691C",
                    },
                  ],
                },
                {
                  nombre: "PEETERS ILONAA MAGDA LENA",
                  grupo: "3",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "692G",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "692H",
                    },
                  ],
                }
              ],
            },
            {
              nombre: "ARQUITECTURA DE COMPUTADORAS 1",
              docentes: [
                {
                  nombre: "ACHA PEREZ SAMUEL",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "651",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "625C",
                    },
                  ],
                },
                {
                  nombre: "BLANCO COCA LETICIA",
                  grupo: "2",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "691B",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "624",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "ALGEBRA II",
              docentes: [
                {
                  nombre: "SALINAS PERICON WALTER OSCAR GONZALO",
                  grupo: "5A",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "690E",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "692A",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "18:45",
                      horaFin: "20:15",
                      aula: "690E",
                    },
                  ],
                },
                {
                  nombre: "SILVA RAMOS HERNAN VICTOR",
                  grupo: "6",
                  dia: [
                    {
                      nombre: "Jueves",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "660",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "691A",
                    },
                    {
                      nombre: "Sábado",
                      horaInicio: "08:15",
                      horaFin: "20:15",
                      aula: "Aux-692A",
                    },
                  ],
                },
                {
                  nombre: "OMONTE OJALVO JOSE ROBERTO",
                  grupo: "8",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "624",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "692D",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "692B",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "CALCULO II",
              docentes: [
                {
                  nombre: "MARTINEZ MAIDA AMILCAR SAUL",
                  grupo: "12",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "612",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "624",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "Aux-617C",
                    },
                  ],
                },
                {
                  nombre: "TERRAZAS LOBO JUAN",
                  grupo: "6",
                  dia: [
                    {
                      nombre: "Miércoles",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "691D",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "625C",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "693A",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "PROGRAMACION",
              docentes: [
                {
                  nombre: "TORRICO BASCOPE ROSEMARY",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "690B",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "INFLAB",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "691A",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "ELEMENTOS DE LA PROGRAMACION Y ESTRUCTURA DE DATOS",
              docentes: [
                {
                  nombre: "TORRICO BASCOPE ROSEMARY",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "617C",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "651",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "690C",
                    },
                  ],
                },
                {
                  nombre: "BLANCO COCA LETICIA",
                  grupo: "2",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "623",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "623",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "690C",
                    },
                  ],
                },
                {
                  nombre: "BLANCO COCA LETICIA",
                  grupo: "3",
                  dia: [
                    {
                      nombre: "Martes",
                      horaInicio: "18:45",
                      horaFin: "20:15",
                      aula: "622",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "691C",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "690C",
                    },
                  ],
                },
              ],
            },
          ],
        },
      ],
    },
    {
      carrera: "LICENCIATURA EN MATEMATICA (DID)",
      semestre: [
        {
          nivel: "1",
          materia: [
            {
              nombre: "BIOLOGIA GENERAL",
              docentes: [
                {
                  nombre: "FERNANDEZ TERRAZAS ERIKA",
                  grupo: "1",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "12:45",
                      horaFin: "14:15",
                      aula: "608A",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "690A",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "11:15",
                      horaFin: "12:45",
                      aula: "608A",
                    },
                    {
                      nombre: "Viernes",
                      horaInicio: "08:15",
                      horaFin: "09:45",
                      aula: "690A",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "FISICA BASICA I",
              docentes: [
                {
                  nombre: "M TORRICO TROCHE MILKA MONICA",
                  grupo: "M",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "624G",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "09:45",
                      horaFin: "11:15",
                      aula: "692B",
                    },
                    {
                      nombre: "Jueves",
                      horaInicio: "06:45",
                      horaFin: "08:15",
                      aula: "692H",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "ALGEBRA I",
              docentes: [
                {
                  nombre: "CARRASCO CALVO ALVARO HERNANDO",
                  grupo: "5",
                  dia: [
                    {
                      nombre: "Lunes",
                      horaInicio: "17:15",
                      horaFin: "18:45",
                      aula: "642",
                    },
                    {
                      nombre: "Martes",
                      horaInicio: "15:45",
                      horaFin: "17:15",
                      aula: "617",
                    },
                    {
                      nombre: "Miércoles",
                      horaInicio: "14:15",
                      horaFin: "15:45",
                      aula: "607",
                    },
                  ],
                },
              ],
            },
            {
              nombre: "CALCULO I",
              docentes: [
                {
                  nombre: "QUISPE CHOQUE VLADIMIR",
                  grupo: "13",
                  dia: [
                  {
                    nombre: "Lunes",
                    horaInicio: "09:45",
                    horaFin: "11:15",
                    aula: "652",
                  },
                  {
                    nombre: "Lunes",
                    horaInicio: "18:45",
                    horaFin: "20:15",
                    aula: "691F",
                  },
                  {
                    nombre: "Martes",
                    horaInicio: "18:45",
                    horaFin: "20:15",
                    aula: "692C",
                  },
                ],
               },
             ],
            }

          ],
        }
      ],    
    },    
  ],
};

export default schedule;
